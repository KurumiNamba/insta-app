    ...
    <ul class="list-group">
        @foreach ($post->comments->take(3) as $comment)
            <li class="list-group-item border-0 p-0 mb-2 bg-dark">
                <a href="{{route('profile.show',$comment->user->id)}}" class="text-decoration-none text-white fw-bold">{{$comment->user->name}}</a>
                &nbsp;
        ...
        @if ($post->comments->count()>3)
            <li class="list-group-item border-0 px-0 pt-0 bg-dark">
                <a href="{{route('post.show', $post->id)}}" class="text-decoration-none small">View All {{$post->comments->count()}} comments</a>
            </li>
        @endif
    </ul>
