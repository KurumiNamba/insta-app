                ...
                <div class="row align-items-center">
                    <div class="col-auto">
                        <a href="{{route('profile.show',$post->user->id)}}">
                            ...
                            @else
                                <i><div class="fa-solid fa-circle-user text-secondary icon-sm"></div></i>
                            @endif
                        </a>
                    </div>
                    <div class="col ps-0">
                            ...
                            @if (Auth::user()->id === $post->user->id)
                                ...
                                <div class="dropdown-menu">
                                    @if ($post->user->isFollowed())
                                        <form action="{{route('follow.destroy', $post->user->id)}}" method="post" class="display-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="border-0 bg-transparent p-0 text-secondary">Following</button>
                                        </form>
                                    @else
                                        <form action="{{route('follow.store', $post->user->id)}}" method="post" class="display-inline">
                                            @csrf
    ...
    <div class="col-4 px-0 bg-dark">
        <div class="card border-0">
                    ...
                    <div class="col-auto">
                        <a href="{{route('profile.show',$post->user->id)}}">
                            @if ($post->user->avatar)
                                <img src="{{$post->user->avatar}}" alt="{{$post->user->name}}" class="rounded-circle avatar-sm">
                            @else
                                <i><div class="fa-solid fa-circle-user text-white icon-sm"></div></i>
                            @endif
                        </a>
                    </div>
                    <div class="col ps-0">
            ...
            <div class="card-body w-100">
                ...
                <div class="row align-items-center">
                    <div class="col-auto">
                        ...
                        @else
                            <form action="{{route('like.store', $post->id)}}" method="post">
                                @csrf
                                <button type="submit" class="btn btn-sm shadow-none p-0 text-white">
                                    <i class="fa-regular fa-heart"></i>
                                </button>
                            </form>
                        @endif
                ...
                <p class="text-danger small">Posted on {{$post->created_at->diffForHumans()}}</p>
                <p class="text-uppercase dark-mode-text small">
                    {{date('M d, Y', strtotime($post->created_at))}}
                    {{-- date(arg1, arg2) --}}
                    {{-- arg1 --> Month Day Year --}}
                    {{-- strtotime(timestamps) --> strtotime('2024-10-10 13:30:30') --}}
                ...
                <div class="mt-3 comment-scroll-bar">
                    ...
                    @if ($post->comments->isNotEmpty())
                        <ul class="list-group mt-2 ">
                            @foreach ($post->comments as $comment)
                                <li class="list-group-item border-0 p-0 mb-2 bg-dark">
                                    <a href="{{route('profile.show',$post->user->id)}}" class="text-decoration-none text-white fw-bold">{{$comment->user->name}}</a>
                                    &nbsp;
                                    <p class="d-inline fw-light text-white">{{$comment->body}}</p>

                                    <form action="{{route('comment.destroy',$comment->id)}}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <span class="text-uppercase dark-mode-text small">{{date('M d, Y', strtotime($comment->created_at))}}</span>
